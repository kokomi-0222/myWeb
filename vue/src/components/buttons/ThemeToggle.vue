...
<template>
  <button class="theme-toggle" @click="toggleTheme" aria-label="切换主题">
    <Transition name="icon-slide" mode="out-in">
      <div v-if="isDark" key="dark" class="icon-wrapper">
        <svg viewBox="0 0 24 24" :width="size" :height="size" class="theme-icon">
          <path
      ...
      <div v-else key="light" class="icon-wrapper">
        <svg viewBox="0 0 24 24" :width="size" :height="size" class="theme-icon">
          <path
...
.icon-slide-enter-from {
  opacity: 0;
  transform: translateY(-50%); /* 从正上方进入 */
}
...
.icon-slide-leave-to {
  opacity: 0;
  transform: translateY(-50%); /* 向正上方退出 */
}
</style>
